...
export default function Dashboard() {
  ...
  const fetchRandomLesson = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Nenhuma lição encontrada. Cadastre uma frase primeiro!",
        );
      }
  ...
  const playReferenceAudio = () => {
    if (currentLesson && currentLesson.audio_path) {
      const audioUrl = `/${currentLesson.audio_path}`;
      ...
      <header className="w-full max-w-3xl flex justify-between items-center mb-10">
        ...
        <div className="flex items-center gap-5">
          ...
          <button
            ...
            className="text-slate-400 hover:text-rose-400 flex items-center gap-2 text-sm font-medium transition-colors border-l border-slate-700 pl-5"
          >
            <LogOut size={18} /> Sair
          </button>
        </div>
      ...
      <main className="w-full max-w-3xl bg-slate-800/50 border border-slate-700 rounded-3xl p-8 shadow-2xl backdrop-blur-sm">
        {isLoadingLesson ? (
          ...
          <>
            ...
            <div
              className={`transition-all duration-500 overflow-hidden ${audioURL ? "opacity-100 max-h-[500px]" : "opacity-0 max-h-0"}`}
            >
              <div className="bg-slate-900/80 rounded-2xl p-6 border border-slate-700/50 flex flex-col items-center gap-4 mb-6">
                ...
                <audio
                  controls
                  src={audioURL}
                  className="w-full max-w-md h-12 rounded-md"
                />
                ...
                {score === null && (
                  ...
                  >
                    {isLoading ? (
                      ...
                      <>
                        <Send size={18} /> Avaliar Pronúncia
                      </>
                    )}
                  </button>
                )}
            ...
            >
              ...
              {Object.keys(phonetics).length > 0 && (
                <div className="mt-6 bg-slate-900/60 rounded-2xl border border-purple-500/20 p-6">
                  <h3 className="text-lg font-semibold text-purple-400 mb-3">
                    🔤 Pronúncia correta
                  </h3>

                  <div className="space-y-3">
                    {Object.entries(phonetics).map(([word, value]) => (
                      <div key={word} className="bg-slate-800 rounded-lg p-3">
                        <div className="font-semibold text-white">{word}</div>

                        <div className="text-cyan-300">IPA: {value.ipa}</div>

                        <div className="text-slate-300">
                          Aproximação: {value.pt_br}
                        </div>
                      </div>
                    ))}
                  </div>
